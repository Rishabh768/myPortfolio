...
function captureScreenshot() {
  ...
  downloadURI("fluid.png", n), URL.revokeObjectURL(n);
}
...
function textureToCanvas(e, r, t) {
  ...
  return o.data.set(e), i.putImageData(o, 0, 0), n;
}
...
function CHECK_FRAMEBUFFER_STATUS() {
  ...
  e != gl.FRAMEBUFFER_COMPLETE && console.trace("Framebuffer error: " + e);
}
let dye,
  velocity,
  divergence,
...
function initFramebuffers() {
  ...
  gl.disable(gl.BLEND),
    (dye =
      null == dye
        ? createDoubleFBO(r.width, r.height, n.internalFormat, n.format, t, a)
        : resizeDoubleFBO(
            dye,
            r.width,
            r.height,
    ...
    (velocity =
      null == velocity
        ? createDoubleFBO(e.width, e.height, i.internalFormat, i.format, t, a)
        : resizeDoubleFBO(
            velocity,
            e.width,
            e.height,
...
function createFBO(e, r, t, n, i, o) {
  ...
  return (
    ...
    gl.clear(gl.COLOR_BUFFER_BIT),
    {
      ...
      texelSizeY: 1 / r,
      attach: (e) => (
        gl.activeTexture(gl.TEXTURE0 + e), gl.bindTexture(gl.TEXTURE_2D, a), e
      ),
    }
  );
}
...
function render(e) {
  config.BLOOM && applyBloom(dye.read, bloom),
    ...
    config.TRANSPARENT || drawColor(e, normalizeColor(config.BACK_COLOR)),
    null == e && config.TRANSPARENT && drawCheckerboard(e),
    drawDisplay(e);
}
